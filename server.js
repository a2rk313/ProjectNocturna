...
app.get("/api/viirs/:year/:month?", async (req, res) => {
  try {
    ...
    if (bbox) {
      try {
        ...
        if (originalRange > 180) {
          ...
          console.log(
            `🌍 Large range detected (${originalRange.toFixed(2)}°), using global data`,
          );
    ...
    console.log(
      `🌐 Calling NASA API: ${url.replace(process.env.NASA_API_KEY, "***")}`,
    );
    ...
    const lines = response.data.split("\n").filter((line) => {
      return (
        line.trim() && !line.startsWith("#") && !line.startsWith("latitude")
      );
    });
    ...
    const avgBrightness =
      brightnessValues.length > 0
        ...
        : 15.3;
    const maxBrightness =
      brightnessValues.length > 0 ? Math.max(...brightnessValues) : 85.2;
    const minBrightness =
      brightnessValues.length > 0 ? Math.min(...brightnessValues) : 0.5;
    ...
    if (error.response) {
      console.error(
        `NASA API Response: ${error.response.status} - ${error.response.statusText}`,
      );
  ...
  try {
    ...
    if (lat && lng) {
      try {
        ...
        console.log(
          "Database query failed, using calculated values:",
          dbError.message,
        );
      }
    }
    ...
    res.json({
      ...
      calculation_based_on:
        lat && lng ? "latitude-based model" : "global average",
    });
  } catch (error) {
...
app.get("/api/sqm-network", async (req, res) => {
  try {
    ...
    const response = await axios.get(
      "https://www.clearskyalarmclock.com/sqm/data.json",
      {
        timeout: 8000,
      },
    );
    ...
    try {
      ...
      const { data, error: dbError } = await supabase
        ...
        .select(
          "id, lat, lng, sqm_reading, bortle_class, measured_at, device_type, is_research_grade",
        )
...
app.post("/api/statistics/region", async (req, res) => {
  try {
    ...
    const stats = {
      ...
      dark_sky_percentage: Math.max(
        0,
        Math.min(100, (baseSQM - 17) * 15),
      ).toFixed(1),
    };

    const interpretation = {
      dark_sky_percentage:
        parseFloat(stats.dark_sky_percentage) > 70
          ? "Excellent dark sky preservation"
          : parseFloat(stats.dark_sky_percentage) > 40
            ? "Moderate light pollution"
            : "Severe light pollution",
      recommendation:
        parseFloat(stats.avg_bortle) > 6
    ...
    res.json({
      ...
      methodology:
        "Statistical analysis based on VIIRS radiance values and ground measurements",
    });
  } catch (error) {
...
app.get("/api/dark-sky-parks", async (req, res) => {
  try {
    ...
    res.json({
      ...
      citation:
        "International Dark-Sky Association. (2024). Dark Sky Place Program.",
...
app.get("/api/measurement", async (req, res) => {
  try {
    ...
    console.log(
      `📍 Measurement request for: ${numLat.toFixed(4)}, ${numLng.toFixed(4)}`,
    );
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        is_research_grade:
          device_type &&
          (device_type.includes("SQM-LE") || device_type.includes("SQM-LU")),
      })
      .select();
...
app.post("/api/viirs/latest", async (req, res) => {
  try {
    ...
    const brightnessValues = Array.from(
      { length: 50 },
    ...
    res.json({
      ...
      area_km2: area.toFixed(2),
      note: process.env.NASA_API_KEY
        ? "Real NASA data used"
        : "Sample data (configure NASA API key)",
    });
  } catch (error) {
...
app.post("/api/ecology/impact", async (req, res) => {
  try {
    ...
    const impacts = {
      avian_migration:
        sqm < 18 ? "High risk" : sqm < 20 ? "Moderate risk" : "Low risk",
      insect_populations:
        sqm < 19
          ? "Severe impact"
          : sqm < 21
            ...
            : "Minimal impact",
      plant_physiology:
        sqm < 20 ? "Disrupted circadian rhythms" : "Normal patterns",
      human_circadian:
        sqm < 18.5
          ...
          : sqm < 20.5
            ...
            : "Minimal disruption",
      sea_turtle_nesting:
        sqm < 19 ? "Critical habitat loss" : "Habitat preserved",
      bat_foraging:
        sqm < 19.5
          ? "Severely impacted"
          : sqm < 21
    ...
    if (sqm < 19)
      recommendations.push("Implement lighting curfews (23:00-05:00)");
    if (sqm < 20) recommendations.push("Install full-cutoff lighting fixtures");
    if (sqm < 21)
      recommendations.push("Use 3000K or lower color temperature LEDs");
    ...
    res.json({
      ...
      area_km2: area.toFixed(2),
      methodology:
        "Based on Gaston et al. (2013) ecological light pollution framework",
      data_sources: [
...
app.post("/api/energy/waste", async (req, res) => {
  try {
    ...
    res.json({
      ...
      savings_potential,
      methodology:
        "Based on DOE lighting energy models and VIIRS radiance data",
      assumptions: [
...
app.get("/api/trends/:lat/:lng", async (req, res) => {
  try {
    ...
    console.log(
      `📈 Trend analysis for: ${numLat.toFixed(4)}, ${numLng.toFixed(4)} over ${yearsInt} years`,
    );
    ...
    res.json({
      ...
      note: isUrban
        ? "Urban area - typically shows light pollution increase"
        : "Rural area - relatively stable",
    });
  } catch (error) {
...
app.post("/api/predictions/advanced", async (req, res) => {
  try {
    ...
    console.log(
      `🤖 Advanced prediction request: ${lat.toFixed(4)}, ${lng.toFixed(4)}`,
    );
    ...
    try {
      ...
      if (!error && data && data.length > 0) {
        ...
        historicalData = Object.keys(yearlyData)
          .map((year) => ({
            year: parseInt(year),
            avg_sqm:
              yearlyData[year].sqms.reduce((a, b) => a + b, 0) /
              yearlyData[year].sqms.length,
            avg_bortle:
              yearlyData[year].bortles.reduce((a, b) => a + b, 0) /
              yearlyData[year].bortles.length,
    ...
    if (models.includes("linear")) {
      predictions.linear = calculateLinearRegression(
        years,
        sqmValues,
        currentYear,
        years_forward,
    ...
    if (models.includes("exponential")) {
      predictions.exponential = calculateExponentialSmoothing(
        years,
        sqmValues,
        currentYear,
        years_forward,
    ...
    if (models.includes("seasonal")) {
      predictions.seasonal = calculateSeasonalTrend(
        years,
        sqmValues,
        currentYear,
        years_forward,
    ...
    if (models.includes("moving_average")) {
      predictions.moving_average = calculateMovingAverage(
        years,
        sqmValues,
        currentYear,
        years_forward,
    ...
    const uncertainty = calculateUncertaintyIntervals(
      sqmValues,
    ...
    res.json({
      ...
      metadata: {
        ...
        data_source: historicalData[0].sample_size
...
function calculateLinearRegression(years, values, currentYear, yearsForward) {
  ...
  return {
    ...
    interpretation: {
      ...
      direction:
        slope > 0.05 ? "improving" : slope < -0.05 ? "worsening" : "stable",
...
function calculateExponentialSmoothing(
  ...
  return {
    ...
    interpretation: {
      ...
      direction:
        trend > 0.05 ? "improving" : trend < -0.05 ? "worsening" : "stable",
...
function calculateSeasonalTrend(years, values, currentYear, yearsForward) {
  ...
  const trendSlope =
    (trend[trend.length - 1] - trend[0]) / (years[years.length - 1] - years[0]);
...
function calculateEnsemble(predictions, yearsForward) {
  ...
  for (let i = 0; i < yearsForward; i++) {
    ...
    const values = models.map((model) =>
      parseFloat(predictions[model].predictions[i].predicted_sqm),
    );
...
function validateModels(years, values, predictions) {
  ...
  Object.keys(predictions).forEach((modelName) => {
    ...
    const historicalPredictions = predictions[modelName].predictions.slice(
      0,
      values.length,
    );
    ...
    const firstPrediction = parseFloat(
      predictions[modelName].predictions[0].predicted_sqm,
    );
...
function calculateUncertaintyIntervals(values, ensemblePredictions) {
  ...
  return ensemblePredictions.predictions.map((pred) => ({
    ...
    lower_bound: (
      parseFloat(pred.predicted_sqm) -
      z95 * historicalStdDev
    ).toFixed(3),
    upper_bound: (
      parseFloat(pred.predicted_sqm) +
      z95 * historicalStdDev
...
async function generateRealTimeSeries(lat, lng, years) {
  ...
  console.log(
    `📊 Fetching real time series for ${lat.toFixed(4)}, ${lng.toFixed(4)} over ${years} years`,
  );
  ...
  try {
    ...
    const realDataCount = data.filter(
      (d) => d.data_source === "NASA VIIRS",
    ).length;
    if (realDataCount >= 3) {
      console.log(
        `✅ Generated time series with ${realDataCount} real data points`,
      );
      return data;
    }
    ...
    console.log(
      `⚠️ Insufficient real data (${realDataCount} years), using enhanced synthetic data`,
    );
...
async function fetchVIIRSDataForYear(lat, lng, year) {
  try {
    ...
    console.log(
      `📊 ${year}: ${dataPoints.length} points, avg brightness: ${avgBrightness.toFixed(2)}, SQM: ${avgSQM.toFixed(2)}`,
    );
...
function interpolateYearValue(existingData, targetYear, lat, lng) {
  ...
  if (previousYear && nextYear) {
    ...
    const interpolatedSQM =
      previousYear.avg_sqm + (nextYear.avg_sqm - previousYear.avg_sqm) * ratio;
    ...
    return {
      ...
      sample_size: Math.floor(
        (previousYear.sample_size + nextYear.sample_size) / 2,
      ),
      data_source: "Interpolated",
      confidence: "medium",
    };
...
function generateEnhancedSyntheticTimeSeries(lat, lng, years, realDataPoints) {
  ...
  if (realDataPoints.length > 0) {
    const avgRealSQM =
      realDataPoints.reduce((sum, d) => sum + d.avg_sqm, 0) /
      realDataPoints.length;
  ...
  console.log(
    `📊 Generated enhanced synthetic time series with ${realDataPoints.length} real data reference points`,
  );
  return data;
}
...
function generateSyntheticTimeSeries(lat, lng, years) {
  console.log(
    "⚠️ Using legacy synthetic data generation - consider upgrading to generateRealTimeSeries()",
  );
  return generateEnhancedSyntheticTimeSeries(lat, lng, years, []);
}
